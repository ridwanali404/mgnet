...
@section('title', 'Transaksi Perusahaan')
@section('style')
...
<div class="ibox">
	...
	<div class="ibox-content">
        <form action="{{ route('admin.transaction.official') }}" method="get">
			<div class="row">
				<div class="col-sm-4">
			...
			<table class="table table-striped table-bordered table-hover transactions">
				...
				<tbody>
					...
					<tr class="gradeX">
						...
						<td>{{ $transaction->user->sponsor->name ?? '' }}</td>
						<td>{{ $transaction->user->sponsor->username ?? '' }}</td>
						...
						<td>
							<div class="text-nowrap">
								<a href="{{ url('product/'.$transaction->product->dash_name) }}" target="_blank">{{ $transaction->product_name }}</a>, <b>Qty</b> {{ $transaction->qty }} pcs {{ $transaction->is_big ? '× ' . $transaction->month . ' bulan' : '' }}. <b>Harga</b> Rp&nbsp;{{ number_format($transaction->product_price) }}. <b>Total</b> Rp&nbsp;{{ number_format($transaction->price) }}.
							</div>
							@if($transaction->is_big && $transaction->product->bigProducts()->count())
							<div class="m-l-md">
								@foreach ($transaction->product->bigProducts as $bigProduct)
								<div class="text-nowrap">
									<a href="{{ url('product/'.$bigProduct->childProduct->dash_name) }}" target="_blank">{{ $bigProduct->product_name }}</a>, <b>Qty</b> {{ $bigProduct->qty }} pcs × {{ $transaction->month  }} bulan.
								</div>
								@endforeach
							</div>
							@endif
...
<div class="modal inmodal" id="confirm{{ $transaction->id }}" tabindex="-1" role="dialog" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content animated fadeInDown">
			<form action="{{ route('official-transaction.confirm', $transaction) }}" method="POST">
				@csrf
				{{ method_field('PUT') }}
				<div class="modal-header">
...
<div class="modal inmodal" id="packed{{ $transaction->id }}" tabindex="-1" role="dialog" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content animated fadeInDown">
			<form action="{{ route('official-transaction.packed', $transaction) }}" method="POST">
				@csrf
				{{ method_field('PUT') }}
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal"><span
...
<div class="modal inmodal" id="shipment{{ $transaction->id }}" tabindex="-1" role="dialog" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content animated fadeInDown">
			<form action="{{ route('official-transaction.shipment', $transaction) }}" method="POST">
				@csrf
				{{ method_field('PUT') }}
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal"><span
