...
@section('content')
    <div class="container-fluid">
        ...
        @if (Auth::user()->type == 'member')
            @php
                $reward_ids = Auth::user()
                    ->userRewards()
                    ->pluck('reward_id');
                $rewards = \App\Models\Reward::orderBy('nominal')->get();
                $cash = Auth::user()->cash_reward;
                $nextReward = \App\Models\Reward::orderBy('nominal')
                    ->whereNotIn('id', $reward_ids)
                    ->first();
                if (!$nextReward) {
                    $nextReward = \App\Models\Reward::orderBy('nominal', 'desc')->first();
                }
                $percentage = round(($cash / $nextReward->nominal) * 100, 2);
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="text-center">
                                <h5 class="d-flex flex-wrap"><span>{{ number_format($cash) }} /
                                        {{ number_format($nextReward->nominal) }}</span><span
                            ...
                            <td class="text-center">{{ number_format($nextReward->nominal) }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
                        ...
                        @foreach ($rewards as $key => $a)
                            @php
                                ...
                                $userReward = Auth::user()
                                    ->userRewards()
                                    ->where('reward_id', $a->id)
                                    ->first();
                                $platinum_count = Auth::user()
                                    ->sponsors()
                                    ->whereHas('userPin', function ($q) {
                                        $q->whereHas('pin', function ($q_pin) {
                                            $q_pin->whereIn('name', ['Platinum', 'Basic Upgrade Platinum', 'Silver Upgrade Platinum', 'Gold Upgrade Platinum']);
                                        });
                                    })
                                    ->count();
                            @endphp
                            <tr>
                                <td class="text-center">{{ number_format($a->nominal) }}</td>
                                <td class="text-center">{{ $a->reward }}</td>
                                <td class="text-center">
                                    @if ($userReward)
                                        @if ($userReward->is_paid)
                                    ...
                                    @elseif(
                                        $a->is_platinum &&
                                            !in_array(Auth::user()->userPin->pin->name, [
                                                'Platinum',
                                                'Basic Upgrade Platinum',
                                                'Silver Upgrade Platinum',
                                                'Gold Upgrade Platinum',
                                            ]))
                                        Silahkan upgrade akun ke <span class="text-danger">Platinum</span>
                                    @elseif(!$a->is_platinum && $platinum_count < $star_used)
                                        Jumlah Platinum pada Sponsor Langsung Anda <span
                                            class="text-danger">{{ $platinum_count }}</span>, silahkan sponsori <span
                                            class="text-danger">{{ $star_used - $platinum_count }}</span> lagi.
                                    @elseif($cash < $a->nominal)
                                        Jumlah Cash belum mencukupi.
                                    @elseif($cash >= $a->nominal)
                                        @php
                                            $before = \App\Models\Reward::where('nominal', '<', $a->nominal)->pluck('id');
                                            $claimed = Auth::user()
                                                ->userRewards()
                                                ->whereIn('reward_id', $before)
                                                ->count();
                                        @endphp
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table id="userReward" class="display nowrap table table-hover table-striped table-bordered"
                        ...
                        <tbody>
                            @foreach ($userRewards as $a)
                                <tr>
                                    <td>{{ $loop->index + 1 }}</td>
                                    <td><code>{{ $a->created_at }}</code></td>
                                    <td>{{ $a->user->username }}</td>
                                    <td>{{ $a->reward->reward }}</td>
                                    @if (Auth::user()->type == 'admin')
                                        <td class="text-right">
                                            <button class="btn btn-xs btn-rounded btn-success" data-toggle="modal"
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table id="userRewardHistories" class="display nowrap table table-hover table-striped table-bordered"
                        cellspacing="0" width="100%">
                        <thead>
                            <tr>
                                <th>#</th>
                        ...
                        <tbody>
                            @foreach ($userRewardHistories as $a)
                                <tr>
                                    <td>{{ $loop->index + 1 }}</td>
                                    <td><code>{{ $a->created_at }}</code></td>
                                    <td>{{ $a->user->username }}</td>
        ...
        @if (Auth::user()->type == 'admin')
            <form class="form-group" method="GET" action="{{ url('user-reward') }}">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <h6 class="card-subtitle">Histori poin reward masuk</h6>
                <div class="table-responsive">
                    <table id="userRewardBonuses" class="display nowrap table table-hover table-striped table-bordered"
                        cellspacing="0" width="100%">
                        <thead>
                            <tr>
                                <th>#</th>
                        ...
                        <tbody>
                            @foreach ($userRewardBonuses as $a)
                                <tr>
                                    <td>{{ $loop->index + 1 }}</td>
                                    <td><code>{{ $a->created_at }}</code></td>
                                    <td>{{ $a->user->username }}</td>
                                    ...
                                    <td class="text-right">
                                        @if (Auth::user()->type == 'admin')
                                            <a class="btn btn-xs btn-rounded btn-success"
                                                href="{{ route('user-reward.index', ['date' => \Carbon\Carbon::parse($a->created_at)->format('Y-m-d'), 'username' => $a->user->username]) }}">detail</a><br />
                                        @else
                                            <a class="btn btn-xs btn-rounded btn-success"
                                                href="{{ route('user-reward.index', ['date' => \Carbon\Carbon::parse($a->created_at)->format('Y-m-d')]) }}">detail</a><br />
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
            ...
            @if ($user && $user->userPin->level >= 3)
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <h6 class="card-subtitle">Potensi poin reward</h6>
                        <ul class="timeline">
                        ...
                        <table class="table table-stripped m-0">
                            <tr>
                                <td class="text-center">{{ number_format($pr_l) }} Poin</td>
                                <td class="text-center">{{ number_format($pr_r) }} Poin</td>
                            </tr>
                        </table>
                    </div>
                </div>
    ...
    @if (Auth::user()->type == 'admin')
        @foreach ($userRewards as $a)
            <form action="{{ url('user-reward/' . $a->id . '/confirm') }}" method="POST">
                @csrf
                @method('PUT')
        ...
        @foreach ($rewards as $a)
            @if ($cash >= $a->nominal)
                ...
                @if ($claimed == $before->count())
                    <form action="{{ url('user-reward/' . $a->id . '/claim') }}" method="POST"
