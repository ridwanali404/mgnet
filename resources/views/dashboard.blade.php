...
@endsection
@section('content')
<form class="form-group" action="{{ url('a/dashboard') }}" method="get">
	<input type="month" id="month" name="month" value="{{ request()->month ?? date('Y-m') }}" class="form-control" onchange="this.form.submit()">
</form>
@php
if (Auth::user()->type == 'admin' || Auth::user()->type == 'cradmin') {
	$types = ['masterstockist', 'stockist', 'general'];
} else {
	$types = ['masterstockist'];
}
@endphp
@foreach ($types as $type)
@php
$t1s = clone $transactions;
if (Auth::user()->type == 'admin' || Auth::user()->type == 'cradmin') {
	$t1s = $t1s->where('type', $type);
} else {
	$t1s = $t1s->where('master_stockist_id', Auth::id());
}
@endphp
@if($t1s->count())
<div class="ibox">
	<div class="ibox-title">
		<h5>
			@if ($type == 'masterstockist')
			Transaksi Master Stockist
			...
			<table class="table table-striped table-bordered table-hover transactions">
				...
				<tbody>
					...
					<tr class="gradeX">
						...
						@if($type == 'general')
						<td>{{ $transaction->sponsor->name ?? '' }}</td>
						<td>{{ $transaction->sponsor->username ?? '' }}</td>
						@endif
						...
						<td>
							...
							<div class="text-nowrap"><a href="{{ url('product/'.$cart->product->dash_name) }}" target="_blank">{{ $cart->name }}</a>, <b>Qty</b> {{ $cart->qty }}
								...
								@if($cart->product->is_big && $cart->product->bigProducts()->count())
								<div class="m-l-md">
									@foreach ($cart->product->bigProducts as $bigProduct)
									<div class="text-nowrap">
										<a href="{{ url('product/'.$bigProduct->childProduct->dash_name) }}" target="_blank">{{ $bigProduct->product_name }}</a>, <b>Qty</b> {{ $bigProduct->qty }} pcs × {{ $cart->product->month  }} bulan.
									</div>
									@endforeach
								</div>
								@endif
...
@foreach ($types as $type)
@php
$t2s = clone $transactions;
if (Auth::user()->type == 'admin' || Auth::user()->type == 'cradmin') {
	$t2s = $t2s->where('type', $type);
} else {
	$t2s = $t2s->where('master_stockist_id', Auth::id());
}
@endphp
@foreach ($t2s as $transaction)
@if($transaction->receipt)
